...
class Abilities {
    ...
    public static function execute_post_from_media( $input ) {
        ...
        // Validate each attachment exists and is an image
        ...
        // Use a system post if no post_id provided
        if ( ! $post_id ) {
            $post_id = self::get_or_create_system_post();
        }

        // Process images with server-side cropping
        $temp_image_urls = [];
    ...
    public static function execute_list_media( $input ) {
        ...
        $search     = $input['search'] ?? '';
        $not_posted = $input['not_posted'] ?? false;
        ...
        foreach ( $query->posts as $attachment ) {
            ...
            // Check if this image has been posted to Instagram
    ...
    public static function execute_auth_status( $input ) {
        ...
        $settings   = get_option( 'pti_settings', [] );
        $expires_at = $settings['token_expires_at'] ?? null;

        // Try to get username from Instagram
        $username = null;
    ...
    private static function process_image_for_instagram( $attachment_id, $crop_config, $post_id, $index ) {
        ...
        // Calculate center crop for target aspect ratio
        ...
        // Ensure minimum Instagram dimensions (320x320) and maximum (1440 on any side)
        ...
        // Ensure JPEG format for Instagram compatibility
    ...
    private static function has_been_posted_to_instagram( $attachment_id ) {
        ...
        // Check all posts for this image in their shared images meta
